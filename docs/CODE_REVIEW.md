...
| 8e-ix-a | Extract `EbonClearance_MainPanel.lua` (MainOptions panel + BuildMainPanel, ~241 LOC moved across 3 non-contiguous chunks). 1 new NS exposure (ResetSession). Five `InterfaceOptionsFrame_OpenToCategory(MainOptions)` call sites in EbonClearance.lua converted to `_G["EbonClearanceOptionsMain"]` lookups. Panel-infra helpers stay in EbonClearance.lua for later sub-stages | **DONE** (commit `<pending>`) |
| 8e-ix-b | Future: extract CreateListUI + list-row factories + panel-infra helpers (MakeHeader, MakeLabel, AddCheckbox, AddSlider, EC_PANEL_WIDTH, EC_UpdatePanelWidth, EC_compCache.initPanel/setPanelWidth/registerWidth/etc., ~2000 LOC). The densest remaining cluster | pending |
