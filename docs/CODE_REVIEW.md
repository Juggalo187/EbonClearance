...
| 8d | Extract `EbonClearance_BagContextMenu.lua` (Alt+Right-Click bag quick-action popup: EC_CTX_ROWS + EC_BuildCtxFrame + EC_ShowItemContextMenu + EC_InstallBagContextHookOnce, ~375 LOC). Three list-mutation helpers (AddItemToList, RemoveItemFromList, FindAddConflict) exposed on NS as Stage 8d prep | **DONE** (commit `<pending>`) |
| 8e | Future: extract remaining UI surface (Interface Options panels, CreateListUI + list-row factories, ProcessBagsPanel UI). The densest cross-file dependency cluster in the addon; deserves careful subdivision | pending |
